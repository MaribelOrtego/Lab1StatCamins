...
Mètode2:  Podem escriure directament a la consola `setwd(ruta del directori)`. Però tu i jo sabem que probablement no farem servir aquest mètode mai.
...
En les properes sessions, si executem R a partir d'un script, podem fer servir el Mètode3 per a fixar el directori de treball:

Mètode3: Si volem establir com a directori de treball el directori on es troba l'script actual, `Session->Set Working Directory->To Souce File Location`
::: 
...
## Segon pas: Llegir dades de fitxer extern
...
Com hem comentat anteriorment, és freqüent instal·lar noves llibreries per tal de millorar les funcionalitats de `R`. En aquest cas, instal·larem i farem servir la llibreria `car`.

## Instal·lació

La gran majoria de llibreries es troben dipositades a CRAN, que és el repositori oficial de R. Les llibreries _guardades_ aquí compleixen uns estàndards de qualitat. Actualment també hi ha desenvolupadors que pugen les seves llibreries a Github i repositoris similars. De fet, per baixar-vos aquest tutorial heu instal·lat una llibreria allotjada a Github. Per a situacions on no hi ha una connexió a internet fiable, les llibreries es poden instal·lar també des d'un fitxer local.

Només cal instal·lar amb un d'aquests mètodes:

Mètode1: Actualment `RStudio` ens avisa de que algun paquet no es troba instal·lat: apareix una franja groga a la part superior de l'script, on podem clicar _instal·la_ .

Mètode2: Aquest és el meu preferit, donat que deixem rastre de la llibreria que hem instal·lat. Es recomana posar les línies de llibreries al principi del nostre script.

```{r instalcar}
# Instal·lació paquets. Només cal fer-ho una vegada
#install.packages("car")
```
La sentència es troba comentada per evitar problemes d'execució al tutorial. Elimina el comentari i instal·la el paquet `car` executant la línia `install.packages()` . 

Mètode3: A la pestanya `Packages` del panell inferior dret tenim el botó Instal·la. Si cliquem, ens permet escriure el nom del paquet que volem instal·lar. Per defecte baixem de CRAN (necessitem internet) i s'instal·la a una ubicació accessible pels privilegis que tinguem a l'ordinador. També per defecte s'instal·laran totes les llibreries de les que es tenen dependències. 

::: {.infobox .caution data-latex="{caution}"}
En alguns casos es poden presentar advertències corresponents a les llibreries. Les més comunes són les de versió i les de nom de funció coincident amb una llibreria prèviament activada. Per defecte, si hi ha coincidència, prima la llibreria activada més recentment.
:::

## Utilització 

La instal·lació es fa una vegada (si més endavant canviem la versió de R, potser també caldrà). Però cada sessió que volguem utilitzar la llibreria, caldrà activar-la.

Ep! Les línies de codi de crida de llibreries, millor al principi de l'script.

```{r libcar, include=TRUE}
#crida a les llibreries. 
library(car)
```

I ara ja podem utilitzar les funcions contingudes a `car`.

::: {.infobox .caution data-latex="{caution}"}
Actualment es recomana indicar de quin paquet procedeix la funció que utilitzem,  per millorar la reproducibilitat del nostre treball, indicant `nompaquet:: nomfuncio (arguments)`
:::

Per exemple, podem dibuixar un diagrama de dispersió entre la variable Alçada i el nombre de Germans

```{r scattercar, include=TRUE}
car::scatterplot(dades$Alt,dades$Germans)
```

Observem que ens apareixen moltes línies que ara no sabem interpretar. Podem modificar la gràfica afegint alguns arguments. De pas, afegim títol i noms als eixos:

```{r scattercarnet, include=TRUE}
car::scatterplot(dades$Alt,dades$Germans, smooth=FALSE, regLine=FALSE, boxplots=FALSE,
            main="Scatterplot", xlab="Alçada", ylab="Nombre de Germans")
```

En les properes sessions utilitzarem diagrames de dispersió per a representar relacions bivariades entre variables.
...
A continuació us mostrem alguns exemples de gràfiques descriptives realitzades amb aquestes llibreries. Més endavant podem dedicar una sessió a explicar-ne el funcionament. Veureu que els resultats són molt atractius, però la sintaxi és una mica més complicada que la del paquet `base`. 
...
Generem un conjunt de dades artificial per a mostrar les representacions gràfiques:
...
Com veieu, les representacions gràfiques obtingudes són molt atractives, però necessiten profunditzar més en l'estètica. A les sessions de Lab prioritzarem treballar els conceptes, no tant l'estètica, i per això generarem les gràfiques amb el paquet `base`.

# Conclussió {-}

Amb aquest apartat hem acabat el contingut del Lab1. Pots tornar enrere per a repassar conceptes, o bé passar al Lab Entregable.
